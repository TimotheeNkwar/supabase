...
def get_articles():
    ...
    response = query.range((page-1)*per_page, page*per_page - 1).order('timestamp', desc=True).execute()
    
    articles = response.data or []
    
    # Pour l'admin, renvoyer content_md comme content
    for article in articles:
        article['content'] = article.get('content_md', article.get('content', ''))
...
def create_article():
    ...
    content_html = custom_markdown(content_md)
    ...
    if response.data:
        return jsonify({'article': response.data[0]}), 201
    else:
        return jsonify({'error': 'Failed to create article'}), 400
...
def update_article(article_id):
    ...
    response = supabase.table('articles').update(payload).eq('id', article_id).execute()
    
    if response.data:
        return jsonify({'article': response.data[0]})
    else:
...
def get_article(article_id):
    if article_id.isdigit():
        response = supabase.table('articles').select('*').eq('id', int(article_id)).single().execute()
    else:
        response = supabase.table('articles').select('*').eq('uuid', article_id).single().execute()
...
@login_required
def toggle_visibility(article_id):
    data = request.get_json()
    hidden = data.get('hidden', True)
    supabase.table('articles').update({'hidden': hidden}).eq('id', article_id).execute()
    return jsonify({'success': True})

@api.route('/categories', methods=['GET'])
def get_categories():
    categories = ["technologie", "trends", "supply-chain", "tutorials", "analysis", "career", "methodology"]
...
def show_article(id):
    try:
        ...
        try:
            ...
            logger.error(f"Error incrementing views for article {id}: {str(e)}")
